...
namespace SerialPortListener.Serial
{
    ...
    public class SerialPortManager : IDisposable
    {
        ...
        {
            // cerco le porte COM presenti sul computer
            ...
            // in base alla modalità simulazione attiva o meno, carico le porte COM
            if (ConfigurationManager.AppSettings["simulazione"] == "OFF")
            {
                // se trovo porte seriali carico quella che trovo settata nel file app.config
                if (_currentSerialSettings.PortNameCollection.Length > 0)
                    _currentSerialSettings.PortName = ConfigurationManager.AppSettings["portaCOM"];
            }
            else
            {
                // oppure carico la prima che becco tra quelle disponibili del sistema (0 nell'array)
                if (_currentSerialSettings.PortNameCollection.Length > 0)
